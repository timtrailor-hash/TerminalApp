...
final class SSHCommandRunner: ObservableObject {
    ...
    private static let validTarget = try! Regex("^[a-zA-Z0-9_:\\-]+$")

    /// Send text to a specific tmux window as if typed.
    func sendToTmux(_ text: String, target: String = "mobile") async {
        guard target.wholeMatch(of: Self.validTarget) != nil else {
            cmdLog.error("Invalid tmux target rejected: \(target.prefix(40))")
            return
        }
    ...
    func sendEnterToTmux(target: String = "mobile") async {
        guard target.wholeMatch(of: Self.validTarget) != nil else { return }
